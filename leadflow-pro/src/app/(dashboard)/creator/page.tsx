...
function CreatorLoading() {
  return (
    <div className="flex h-full w-full bg-slate-950/40 backdrop-blur-md luxury-gradient">
      <div className="w-80 border-r border-white/5 bg-white/2 p-6 space-y-6">
      ...
      <div className="flex-1 p-12 flex flex-col items-center justify-center space-y-6">
         <div className="w-24 h-24 rounded-3xl bg-white/2 border border-white/5 flex items-center justify-center animate-pulse">
...
function CreatorError({ error, onRetry }: { error: Error | null; onRetry?: () => void }) {
  return (
    <div className="flex h-full items-center justify-center p-8 bg-slate-950 luxury-gradient">
      <div className="max-w-md w-full p-10 glass-panel border-primary/20 bg-primary/5 text-center shadow-2xl rounded-4xl">
...
export default function CreatorPage() {
  return (
    <div className="h-[calc(100vh-8rem)] -m-8 relative overflow-hidden">
      <ErrorBoundary
